...
inline std::string Logger::createLogFileName() {
...
inline std::string Logger::getLogLevelString(LogLevel level) {
    if (!useColors) {
        switch (level) {
...
inline void Logger::enableTimestamps(bool enable) {
    showTimestamps = enable;
}

inline void Logger::enableSourceInfo(bool enable) {
    showSourceInfo = enable;
}

inline void Logger::enableColors(bool enable) {
    useColors = enable;
}

inline void Logger::setBasePath(const std::string& path) {
    basePath = path;
...
inline std::string getShortFilePath(const std::string& basePath, const std::string& filePath) {
...
inline void Logger::logInternal(LogLevel level, const std::string& message, const char* file, int line) {
    if (!initialized && !init()) {
